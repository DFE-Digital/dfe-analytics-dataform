...
<details> <summary>scan_for_posible_unhidden_sensitive_pii_in_field_names.sh</summary>
#!/bin/bash

# Set variables - please update your-project-id-here before running!
PROJECT_ID="your-project-id-here" 
POLICY_TAG_REGEX="hidden"

# Enable nocasematch option for case-insensitive matching
shopt -s nocasematch

# Fetch list of datasets in the project
datasets=$(bq ls --project_id "$PROJECT_ID" --format=prettyjson | jq -r '.[].datasetReference.datasetId')

# Iterate over each dataset
for dataset in $datasets; do
  # Skip datasets with names beginning with "dataform" except "dataform" itself
  if [[ "$dataset" == dataform* ]] && [[ "$dataset" != "dataform" ]]; then
  ...
  tables=$(bq ls --dataset_id "$dataset" --project_id "$PROJECT_ID" --format=prettyjson | jq -r '.[].tableReference.tableId')

  # Iterate over each table and check fields
  for table in $tables; do
    echo "  Checking table: $table"
    
    # Get table schema
    schema=$(bq show --project_id "$PROJECT_ID" --format=prettyjson "$dataset.$table" | jq -r '.schema.fields')

    # Check if schema is null or empty
    if [ -z "$schema" ] || [ "$schema" == "null" ]; then
      echo "    No schema available for table '$table' in dataset '$dataset'"
      continue
    fi
    
    # Iterate over each field in the table schema
    echo "$schema" | jq -c '.[]' | while read -r field; do
      # Safely extract fieldName and ensure it exists
      fieldName=$(echo "$field" | jq -r 'if .name then .name else empty end')
      ...
      # Safely extract policyTags
      policyTags=$(echo "$field" | jq -r 'if .policyTags.names then .policyTags.names[] else empty end')

      # Check if field name matches the regular expression
      if [[ $fieldName =~ (email|name|dob|birth|national|insurance|trn|phone|postcode|address|mobile|passport|driv|ethni|religio|union|sex|gender|orientation|disabilit) ]]; then
        # Check if the field's policy tags do not match the policy tag regex
        if ! echo "$policyTags" | grep -Eq "$POLICY_TAG_REGEX"; then
          echo "    Field '$fieldName' in table '$table' (dataset '$dataset') matches list of phrases indicating possible sensitive PII and does not have policy tag"
        fi
      fi
    done
  done
