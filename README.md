...
- ```hideCreatedAt``` - boolean; optional; table level. If ```true``` the ```created_at``` field for this table will have the policy tag at ```hiddenPolicyTagLocation``` attached to it.
- ```hideUpdatedAt``` - boolean; optional; table level. If ```true``` the ```updated_at``` field for this table will have the policy tag at ```hiddenPolicyTagLocation``` attached to it.
...
dfe-analytics-dataform also contains two assertions which will fail to alert you when the data being streamed into the ```data``` and ```hidden_data``` fields in the ```events``` table does not match the configuration specified in the parameters above:
...
- ```hidden_pii_configuration_does_not_match_sample_of_historic_events_streamed``` will fail when some events in a sample representing 1% of historic events did not match your hidden field configuration. To prevent this failure, either (a) delete historic events from your source ```events``` table with ```event_type``` ```create_entity```, ```update_entity```, ```delete_entity``` or ```import_entity``` which have data in the ```data``` field that should be in the ```hidden_data``` field or (b) update these events to migrate the fields in ```data``` into ```hidden_data```.

If either of these assertions, most tables generated by dfe-analytics-dataform will not be updated. This is essential to ensure that data hidden fields is not copied into non-hidden fields in other tables generated by ```dfe-analytics-dataform```.
