...
As of v2.5.0, `dfe-analytics-dataform` also supports ingestion of entity data from [Airbyte](https://airbyte.com/) as a data source alongside the existing `dfe-analytics` event stream. To opt in to this feature, set `enableAirbyteSource: true`. By default, no Airbyte-related tables or assertions will be generated, and existing `dfe-analytics` processing will be unaffected.
...
13. If you are using Airbyte, add the following parameters to your existing `dfeAnalyticsDataform()` configuration block in `definitions/dfe_analytics_dataform.js` to enable Airbyte processing:
...
dfeAnalyticsDataform({
  ...
  airbyteConfig: {
    datasetName: "name of the BigQuery dataset that your Airbyte table is located in"
  },
  airbyteHeartbeat: {
    datasetName: "name of the BigQuery dataset that your Airbyte Heartbeat table is located in",
    tableName: "name of your airbyte_heartbeat table, defaults to airbyte_heartbeat"
  }
...
- ```bqProjectName``` - name of the BigQuery project that your events table is located in. Defaults to the same BigQuery project configured in your GCP Dataform release configuration.
- ```bqDatasetName``` - name of the BigQuery dataset that your events table is located in. Defaults to the same BigQuery dataset configured in your GCP Dataform release configuration.
...
- ```customEventSchema``` - formatted similarly to ```dataSchema```. Used to configure transformation of ```dfe-analytics``` custom events. See "Updating your ```dataSchema``` and ```customEventSchema``` configurations" section below for details.
...
- ```enableWebRequestIdentityInference``` - whether to generate the server side web request identity inference tables. When enabled, the pipeline applies a deterministic identity resolution methodology to produce an inferred identity field for web analytics. This inferred identity represents the best approximation possible from the available data and should not be considered a verified or true identity. The inferred identity output is intended to be used to construct the ```session_details``` table if it is also enabled. Boolean (```true``` or ```false```, without quotes). Defaulted to ```false``` if not specified as this requires configuration to work.
- ```enableSessionDetailsTable``` - whether to generate the final ```session_details``` table. When ```enableWebRequestIdentityInference``` is enabled, sessions use the inferred analytics identity output produced by the server side deterministic identity inference process; otherwise, sessions are constructed using the anonymous device identifier ```anonymised_user_agent_and_ip```. Boolean (```true``` or ```false```, without quotes). Defaults to ```true``` if not specified.
...
- ```attributionParameters``` - list of parameters to extract from the request query as individual fields in funnel analysis (for funnels which are new traffic to the site only). Defaults to ```['utm_source', 'utm_campaign', 'utm_medium', 'utm_content', 'gclid', 'gcsrc','fbclid','dclid','gad_source','ds_rl','gbraid','msclkid']``` if not specified.
...
- ```socialRefererDomainRegex``` - [re2](https://github.com/google/re2/wiki/Syntax)-formatted regular expression to use to work out whether an HTTP referer's domain name is a social media site. Defaults to ```'(?i)(facebook|twitter|^t.co|linkedin|youtube|pinterest|quora|wechat|weibo|whatsapp|tumblr|reddit|x.com|instagram|tiktok|messenger|telegram|douyin|qq|qzone|josh|teams|skype|tieba|threads|viber|imo|xiaohongshu|line|picsart|bluesky)'``` if not specified.
...
## Updating your ```dataSchema``` and ```customEventSchema``` configurations
...
Update your ```customEventSchema``` configuration whenever you wish to transform dfe-analytics custom events into a flattened table for each custom event type.

Once you have updated your dataSchema, commit your changes, merge to main/master and then re-run your pipeline in Dataform.

If you update your ```customEventSchema```, this pipeline run must 'full refresh' the relevant incremental tables in Dataform. See [Dataform documentation](https://cloud.google.com/dataform/docs/incremental-tables) for more information.
...
```customEventSchema``` shares the same format, except that parameters marked with an asterisk (*) above are not supported for custom events, and the ```eventType``` parameter must also be specified - this is the ```event_type``` of custom events configured in dfe-analytics.
...
```dfe-analytics-dataform``` will not require you to update ```customEventSchema```. However, if your custom event schema changes you may wish to update ```customEventSchema``` to match.

### Retaining access to data in renamed fields
If a field in your database or custom event has been renamed one or more times, and if the data type of that field has not changed, you may merge data from that field stored under its previous names by adding the configuration ```pastKeyNames: ['past_key_name_1','past_key_name_2']``` to the key configuration for that field. The value of that field will be handled as if it were called ```keyName``` if a field with that name is present. If it is not present then the value will be set to the first value in ```pastKeyNames```. If no field with that name is present then the value will be set to the second value in ```pastKeyNames```, and so on. (Behind the scenes, this functions as a SQL ```COALESCE()```.)

### Retaining access to historic fields
If a field used to be included in custom event data or streamed entity event data for a particular table, but is no longer streamed, you may retain access to that data in historic versions of entities/events in that table by adding the configuration ```historic: true``` to the key configuration for that field.

This may be useful if a field has been deleted from a table in the database or custom event, but you wish to continue to analyse past versions of that table/event which do contain that field, or changes between past versions of that table/event. For entities, the field will contain a ```NULL``` value for the latest version of the table.
...
Similar monitoring for custom event data is not supported because there is no source database which this data can be compared to.

When Airbyte is enabled, a separate freshness check is available via the heartbeat table. If the Airbyte heartbeat table has not been updated within the configured ```freshnessHours``` (in ```airbyteHeartbeat```), the ```{eventSourceName}_airbyte_global_data_not_fresh``` assertion will fail.
...
- ```hidePrimaryKey``` - boolean; optional; table level. If ```true``` the primary key for this table will have the policy tag at ```hiddenPolicyTagLocation``` attached to it. The primary key is ```id``` by default, but may be configured via the table-level ```primaryKey``` parameter. Not supported for custom events as they do not have primary keys.
- ```hidden``` - boolean; optional; key level. Set this to ```true``` for an individual key within a table or custom event to ensure that this field has a policy tag attached to it.
...
- ```hidden_pii_configuration_does_not_match_entity_events_streamed_yesterday``` will fail when events were streamed since the beginning of yesterday which did not match your hidden field configuration. To prevent this failure, change your ```dfe-analytics``` configuration (analytics_hidden_pii.yml file) and/or your ```dfe-analytics-dataform``` configuration (parameters above) so that they match.
...
When Airbyte is enabled, the ```{entity}_airbyte_fields_not_in_schema_{eventSourceName}``` assertion serves a similar purpose — it blocks the pipeline if any field in the Airbyte source table has not been explicitly classified with ```hidden: true``` or ```hidden: false``` in your ```dataSchema```. This prevents accidental exposure of sensitive data through the Airbyte output tables.
...
To configure a data retention schedule for data processed by ```dfe-analytics-dataform``` add expirationDays and/or ```webRequestEventExpirationDays``` parameter(s) to your ```dfeAnalyticsDataform()``` in Dataform as in the following example:

```
dfeAnalyticsDataform({
  eventSourceName: "patent-a-biscuit-type",
  bqEventsTableName: "events",
  urlRegex: "www.patent-a-biscuit-type.gov.uk",
  expirationDays: 2555, // set to the integer number of days you want to delete data in your BigQuery project after, or leave as false if you don't want to enforce a project-wide data retention schedule
  webRequestEventExpirationDays: 365, // set to the integer number of days you want to delete web requests in your events table after
  dataSchema: [{
    entityTableName: "biscuit_types",
    expirationDays: 365, // set to the integer number of days you want to delete entity events for this entityTableName after
    keys: [{
      keyName: "name",
      dataType: "string"
    }, {
      keyName: "contains_chocolate",
      dataType: "boolean"
    }, {
...
If a data retention schedule has been configured via the top-level, custom event-level or entity-level ```expirationDays``` parameter and/or ```webRequestEventExpirationDays``` parameter, ```dfe-analytics-dataform``` will delete data in tables generated by ```dfe-analytics-dataform``` to the extent that it is possible to prove that the data was last updated the appropriate number of days ago. This includes deleting:
...
- Pageviews and sessions in the ```pageview_with_funnels```, ```sessions``` and ```session_details``` tables which are older than ```expirationDays``` or ```webRequestEventExpirationDays``` (whichever is more recent)
...
- When Airbyte is enabled, versions in the Airbyte ```_version``` tables which are older than either the top-level or entity-level ```expirationDays``` parameter (whichever is more recent)
...
- For each ```eventType``` you specified in ```customEventSchema``` like ```bar```, an incremental table called something like ```bar_foo```. ```bar_foo``` is a denormalised ('flattened') version of ```events_foo```, flattened according to the schema for ```foo``` you specified in ```customEventSchema```. Metadata will be set to match the descriptions set in ```customEventSchema```.
...
- A set of web request identity inference tables in the ```web_analytics_staging_tables``` dataset. These tables implement the deterministic identity resolution pipeline used to infer analytics identities from web request events. The intermediate tables are required to support the operation of the pipeline, including processing steps that cannot be performed directly on incremental datasets. They also provide transparency for audit, quality assurance and operational monitoring. The tables are: ```identity_anchors_all_foo```, ```identity_auid_state_foo```, ```identity_classify_events_foo```, ```identity_conservative_recursive_walk_input_foo```, ```identity_conservative_recursive_walk_run_foo```, ```identity_conservative_recursive_walk_foo```, ```identity_activity_windows_foo```, ```identity_second_recursive_walk_input_foo```, ```identity_second_recursive_walk_run_foo```, ```identity_second_recursive_walk_foo``` and ```identity_solved_events_foo```. Disabled if ```enableWebRequestIdentityInference``` is ```false```.
- A table called ```session_details_foo```, which contains rows representing web analytics sessions with attribution fields, such as ```medium``` and ```session_referer_domain```, and an array of page views that occurred within each session. Sessions continue while successive observable page requests occur within 30 minutes. A request may also continue the preceding session after a longer gap of up to 3 hours where its referrer exactly matches the preceding page, providing a bounded allowance for unobserved page reading time. When ```enableWebRequestIdentityInference``` is enabled, the table uses the inferred analytics identity output produced by the deterministic identity resolution process. Otherwise, sessions are constructed using the anonymous device identifier ```anonymised_user_agent_and_ip```. Disabled if ```enableSessionDetailsTable``` is ```false```.
...
### Additional tables and assertions created when Airbyte is enabled

When ```enableAirbyteSource: true``` is set, the following additional tables and assertions are created for each entity in your ```dataSchema```. For example if your ```eventSourceName``` is ```foo``` and you have an entity ```bar```:

- An incremental table called ```bar_version_foo_airbyte```, containing the version history of the ```bar``` entity derived from Airbyte. Includes ```valid_from```, ```valid_to```, ```is_current```, ```is_deleted```, and ```version_number``` fields.
- A table (or view, depending on ```materialisation```) called ```bar_latest_foo_airbyte```, containing only the current, non-deleted version of each ```bar``` entity, sourced from the version table.
- An assertion called ```bar_airbyte_fields_not_in_schema_foo``` which **blocks** the version and latest tables if the Airbyte source table contains columns not configured in ```dataSchema```. You must add any new fields to your ```dataSchema``` with an explicit ```hidden: true``` or ```hidden: false``` classification to unblock the pipeline.
