  ...
  /* Generates SQL that creates fields which extract the value of each URL query parameter from the request_query ARRAY of STRUCTs, as long as that parameter's key is in the configuration parameter attributionParameters */
    ...
    attributionParameters.forEach(param => {
      if(['occurred_at','event_type','environment','namespace','request_user_id','request_uuid','request_method','request_path','request_path_grouped','request_user_agent','request_referer','request_query','request_referer_query','response_content_type','response_status','anonymised_user_agent_and_ip','device_category','browser_name','browser_version','operating_system_name','operating_system_vendor','operating_system_version','newly_arrived','next_step'].includes(param)) {
        ...
        sqlToReturn += `IF(REGEXP_CONTAINS(request_referer, "${params.urlRegex}") IS FALSE, NULLIF((SELECT STRING_AGG(value, ",") FROM UNNEST(request_query) WHERE key = "${param}"),""), NULL) AS ${param},\n`;
      }
      })
    return sqlToReturn;
  }
...
${stepFields(params.funnelDepth)}
  IF(REGEXP_CONTAINS(request_referer, "${params.urlRegex}") IS FALSE, "Newly arrived traffic", "Traffic already on site") AS newly_arrived,
  ...
  CASE
    ...
    WHEN (
      request_referer IS NOT NULL
      AND NOT REGEXP_CONTAINS(request_referer, "${params.searchEngineRefererRegex}")
      ${ctx.when(params.attributionParameters.includes('utm_medium'),`OR utm_medium = "referral"`,``)}
    ) THEN "Referral"
    WHEN REGEXP_CONTAINS(request_referer, "${params.searchEngineRefererRegex}")
    ${ctx.when(params.attributionParameters.includes('utm_medium'),`OR utm_medium = "organic"`,``)} THEN "Organic"
    ELSE "Direct or unknown"
  END AS medium,
  IF(REGEXP_CONTAINS(request_referer, "${params.urlRegex}") IS FALSE,SPLIT(request_referer, "/")[SAFE_OFFSET(2)],NULL) AS referer_domain
FROM
