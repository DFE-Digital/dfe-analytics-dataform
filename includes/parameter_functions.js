const validTopLevelParameters = ['eventSourceName',
  ...
  'enableSessionTables',
  'enableWebRequestIdentityResolution',
  'enableSessionDetailsTable',
  'enableMonitoring',
...
function validateParams(params) {
    ...
    if (!/^[A-Za-z0-9_]*$/.test(params.eventSourceName)) {
        ...
        // this will allow webRequestEventExpirationDays = 0, is this intentional?
    ...
    params.dataSchema.forEach(tableSchema => {
        ...
        tableSchema.keys.forEach(key => {
            ...
            } else if (['entity_id', 'new_value', 'previous_value', 'key_updated', 'update_id', 'previous_occurred_at', 'seconds_since_previous_update', 'seconds_since_created', 'previous_event_type'].includes(key.alias || key.keyName)) {
...
function attributionParamFields(params) {
    ...
    var sqlToReturn = '';
    params.attributionParameters.forEach(param => {
        if (['occurred_at', 'event_type', 'environment', 'namespace', 'request_user_id', 'request_uuid', 'request_method', 'request_path', 'request_path_grouped', 'request_user_agent', 'request_referer', 'request_query', 'request_referer_query', 'response_content_type', 'response_status', 'anonymised_user_agent_and_ip', 'device_category', 'browser_name', 'browser_version', 'operating_system_name', 'operating_system_vendor', 'operating_system_version', 'newly_arrived', 'next_step'].includes(param)) {
            ...
            sqlToReturn += `IF(request_referer_domain IS NULL OR NOT REGEXP_CONTAINS(request_referer_domain, "${params.urlRegex}"), NULLIF((SELECT STRING_AGG(IF(STARTS_WITH(TO_JSON_STRING(value), '['), ARRAY_TO_STRING(JSON_VALUE_ARRAY(TO_JSON_STRING(value)), ','), JSON_VALUE(TO_JSON_STRING(value))), ',') FROM UNNEST(request_query) WHERE key = "${param}"),""), NULL) AS ${param},\n`;
        }
    })
    return sqlToReturn;
}
