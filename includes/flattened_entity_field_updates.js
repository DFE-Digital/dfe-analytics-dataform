...
const getOriginalColumnDescriptions = (keys) => {
    return keys.map(key => ({
      ["original_" + (key.alias || key.keyName)]: "Original value when first created of: " + key.description
    })
  )
};
module.exports = (params) => {
  return params.dataSchema.forEach(tableSchema => publish(tableSchema.entityTableName + "_field_updates_" + params.eventSourceName, {
    ...
    columns: Object.assign({
      ...
      }, ...getNewColumnDescriptions(tableSchema.keys), ...getPreviousColumnDescriptions(tableSchema.keys), ...getOriginalColumnDescriptions(tableSchema.keys))
  }).query(ctx => `SELECT
  * EXCEPT(new_DATA, previous_DATA, original_DATA),
  ${tableSchema.keys.map(key => {
        ...
        else if(key.dataType == 'boolean') {
          return `SAFE_CAST(${data_functions.eventDataExtract("new_DATA",key.keyName)} AS BOOL) AS new_${key.alias || key.keyName},\n
          SAFE_CAST(${data_functions.eventDataExtract("previous_DATA",key.keyName)} AS BOOL) AS previous_${key.alias || key.keyName},\n
          SAFE_CAST(${data_functions.eventDataExtract("original_DATA",key.keyName)} AS BOOL) AS original_${key.alias || key.keyName},`;
        } else if (key.dataType == 'timestamp') {
          return `${data_functions.eventDataExtractTimestamp("new_DATA",key.keyName)} AS new_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtractTimestamp("previous_DATA",key.keyName)} AS previous_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtractTimestamp("original_DATA",key.keyName)} AS original_${key.alias || key.keyName},`;
        } else if (key.dataType == 'date') {
          return `${data_functions.eventDataExtractDate("new_DATA",key.keyName)} AS new_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtractDate("previous_DATA",key.keyName)} AS previous_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtractDate("original_DATA",key.keyName)} AS original_${key.alias || key.keyName},`;
        } else if (key.dataType == 'integer') {
          return `SAFE_CAST(${data_functions.eventDataExtract("new_DATA",key.keyName)} AS INT64) AS new_${key.alias || key.keyName},\n
          SAFE_CAST(${data_functions.eventDataExtract("previous_DATA",key.keyName)} AS INT64) AS previous_${key.alias || key.keyName},\n
          SAFE_CAST(${data_functions.eventDataExtract("original_DATA",key.keyName)} AS INT64) AS original_${key.alias || key.keyName},`;
        } else if (key.dataType == 'integer_array') {
          return `${data_functions.eventDataExtractIntegerArray("new_DATA",key.keyName)} AS new_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtractIntegerArray("previous_DATA",key.keyName)} AS previous_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtractIntegerArray("original_DATA",key.keyName)} AS original_${key.alias || key.keyName},`;
        } else if (key.dataType == 'float') {
          return `SAFE_CAST(${data_functions.eventDataExtract("new_DATA",key.keyName)} AS FLOAT64) AS new_${key.alias || key.keyName},\n
          SAFE_CAST(${data_functions.eventDataExtract("previous_DATA",key.keyName)} AS FLOAT64) AS previous_${key.alias || key.keyName},\n
          SAFE_CAST(${data_functions.eventDataExtract("original_DATA",key.keyName)} AS FLOAT64) AS original_${key.alias || key.keyName},`;
        } else if (key.dataType == 'string' || key.dataType == undefined) {
          return `${data_functions.eventDataExtract("new_DATA",key.keyName)} AS new_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtract("previous_DATA",key.keyName)} AS previous_${key.alias || key.keyName},\n
          ${data_functions.eventDataExtract("original_DATA",key.keyName)} AS original_${key.alias || key.keyName},`;
        } else {
