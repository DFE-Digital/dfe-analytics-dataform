...
WITH
  ...
  SELECT
    ...
    IFNULL(SUM(number_of_missing_rows), CASE WHEN SUM(database_row_count) > 0 THEN 0 END) AS number_of_missing_rows,
    IFNULL(SUM(number_of_extra_rows), CASE WHEN SUM(database_row_count) > 0 THEN 0 END) AS number_of_extra_rows,
