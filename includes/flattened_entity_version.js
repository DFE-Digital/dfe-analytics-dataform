...
module.exports = (params) => {
  ...
  ${tableSchema.keys.map(key => {
        if(['valid_from','valid_to','event_type','id','created_at','updated_at','request_uuid','request_path','request_user_id','request_method','request_user_agent','request_referer','request_query','response_content_type','response_status','anonymised_user_agent_and_ip','device_category','browser_name','browser_version','operating_system_name','operating_system_vendor','operating_system_version'].includes(key.alias || key.keyName)) {
          ...
          return `SAFE_CAST(${data_functions.eventDataExtract("DATA",key.keyName)} AS BOOL) AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'timestamp') {
          return `${data_functions.eventDataExtractTimestamp("DATA",key.keyName)} AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'date') {
          return `${data_functions.eventDataExtractDate("DATA",key.keyName)} AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'integer') {
          return `SAFE_CAST(${data_functions.eventDataExtract("DATA",key.keyName)} AS INT64) AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'integer_array') {
          return `${data_functions.eventDataExtractIntegerArray("DATA",key.keyName)} AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'float') {
          return `SAFE_CAST(${data_functions.eventDataExtract("DATA",key.keyName)} AS FLOAT64) AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'string' || key.dataType == undefined) {
          return `${data_functions.eventDataExtract("DATA",key.keyName)} AS ${key.alias || key.keyName},`;
        } else {
