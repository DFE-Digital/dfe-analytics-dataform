...
module.exports = (params) => {
  ...
  event_type,
  entity_id AS id,
  ${data_functions.stringToTimestamp(`DATA_struct.created_at`)} AS created_at,
  ${data_functions.stringToTimestamp(`DATA_struct.updated_at`)} AS updated_at,
  request_uuid,
  request_path,
  request_user_id,
  request_method,
  ...
  ${tableSchema.keys.map(key => {
        if(key.dataType == 'boolean') {
          return `SAFE_CAST(DATA_struct.${key.alias || key.keyName} AS BOOL) AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'timestamp') {
          return `${data_functions.stringToTimestamp(`DATA_struct.${key.alias || key.keyName}`)} AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'date') {
          return `${data_functions.stringToDate(`DATA_struct.${key.alias || key.keyName}`)} AS ${key.alias || key.keyName},`;
        } else if (key.dataType == 'integer') {
          return `SAFE_CAST(DATA_struct.${key.alias || key.keyName} AS INT64) AS ${key.alias || key.keyName},`;
