...
module.exports = (params) => {
  ...
  ${tableSchema.keys.map(key => {
        if(['valid_from','valid_to','event_type','id','created_at','updated_at','request_uuid','request_path','request_user_id','request_method','request_user_agent','request_referer','request_query','response_content_type','response_status','anonymised_user_agent_and_ip'].includes(key.alias || key.keyName)) {
          throw new Error(`'${key.keyName}' is the same as a field name in this table generated by dfe-analytics-dataform, so would produce a table with two columns with the same name. Set 'alias: "alternative_name"' for this field in your dataSchema to prevent this error.`);
        } 
